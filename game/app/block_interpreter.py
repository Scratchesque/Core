...
class Interpreter:
    ...
    def _format_spec_class(self, spec):
        class_font = self._type_to_font(
            f"{self.GAP}{(spec.label).replace(' ','')}", spec.action
        )
...
class InterpreterPanel(UIPanel, Interpreter):
    ...
    def create_ui(self):
        ...
        self.text_box = UITextBox(
            relative_rect=Rect((padding - 5, padding - 5), text_size),
            html_text="",
    ...
    def _update_button_pos(self):
        ...
        self.pull_button.set_relative_position(
            (left - self.PULL_BUTTON_SIZE[1] - 10, center)
        )
            ...
            if "loop" == block.spec.id:
                ...
                for child in block.children_blocks:
                    self.block_list.append(
                        self._format_spec_code(child.spec, func_gap=True)
                    )
            else:
